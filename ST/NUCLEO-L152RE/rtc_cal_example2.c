...
void init_RTC_calendar(char new_time[], char new_date[]);
...
uint8_t cnt = 0;
uint8_t timedate_ready = 0;
uint8_t read = 0;

#define SPACE 8
#define SETTINGSREADY 1
#define SETTINGSON 2
/**
**===========================================================================
**
...
int main(void) {
	...
	while(1) {
		...
		if (timedate_ready == SETTINGSREADY) {
			// Get time from start
			for (int i = 0; i < SPACE; i++) {
				new_time[i] = usart_buffer[i];
			}
			int ii = 0;
			// Get date after space
			for (int i = SPACE+1; usart_buffer[i] != '\0'; i++) {
				new_date[ii] = usart_buffer[i];
				ii++;
			}
			
			// Save years (20)21
			current_thousand_year[0] = new_date[6];
			current_thousand_year[1] = new_date[7];

			
			init_RTC_calendar(new_time, new_date);
			timedate_ready = SETTINGSON;
		}
		else if (timedate_ready == SETTINGSON) {
			...
			USART2_write_string("Please set time and date!\n\r"
								"Format is {19:14:00 25.09.2021}\n\r");
		}
...
void USART2_IRQHandler(void) {
	...
	if(USART2->SR & (1 << 5)) {			//if data available in DR register. p737
			c = USART2->DR;
			if (c == '{') { 			// '{' start
				read = 1;
			}
				...
				usart_buffer[cnt] = c;	// Read characters to buffer if current settings are met
				cnt++;
			}	
	}
}
...
void USART2_Init(void)
{
	...
	USART2->CR1 |= (1 << 5); 	// Enable interrupt
